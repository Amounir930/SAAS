...
export async function POST(request: NextRequest) {
  try {
    ...
    await db.transaction(async (tx) => {
        ...
        newContactId = insertedContact.id;
    ...
    if (!finalContact) {
        return NextResponse.json({ error: 'Contact creation verification failed' }, { status: 500 });
    }

    const { contactTags: _, ...restContact } = finalContact;
