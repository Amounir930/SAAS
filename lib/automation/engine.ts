...
import { WhatsAppProvider, SendMediaPayload, SendTextPayload } from '../whatsapp/types';
...
const ConditionSchema = z.object({
  ...
  type: z.enum(['text', 'number', 'time', 'variable', 'funnel_stage', 'tag']),
  operator: z.enum(['equals', 'not_equals', 'contains', 'starts_with', 'ends_with', 'greater_than', 'less_than', 'gte', 'lte', 'between']),
  value: z.string().optional(),
  value2: z.string().optional(),
});

const StartNodeDataSchema = z.object({
  triggerType: z.enum(['exact_match', 'contains', 'first_message', 'fallback']),
...
async function fileToBase64(filePath: string): Promise<string | null> {
    try {
        ...
        if (!absolutePath.startsWith(path.resolve(process.cwd(), 'public'))) {
...
/**
 * Hardened condition evaluator with strict type safety and schema validation.
 */
function evaluateCondition(
  ...
  try {
    ...
    const { type, operator, value, value2 } = parsed;
    ...
    if (type === 'variable') {
      const varValue = variables[targetValue];
      return !!varValue;
    }
...
/**
 * Expert Automation Engine (Hardened)
 * Processes incoming triggers and manages state machine transitions.
 */
export async function processAutomation(
  ...
  try {
    // 1. Validation
    ProcessAutomationInputSchema.parse({ teamId, chatId, remoteJid, incomingText, instanceId });
    const text = incomingText.trim();
    
    const provider = await getWhatsAppProvider(instanceId);
    ...
    if (!session) {
      ...
      for (const automation of activeAutomations) {
          ...
          const data = StartNodeDataSchema.parse(startNode.data);
      ...
      if (finalAutomation) {
          const flow = FlowDataSchema.parse({ 
            nodes: finalAutomation.nodes, 
            edges: finalAutomation.edges 
          });
          ...
          if (startNode) {
              ...
              const builtinVars: Record<string, string> = {
                ...
                contact_agent: (fullContact as any)?.assignedUser?.name || (fullContact as any)?.assignedUser?.email || 'Unassigned',
                contact_department: (fullContact as any)?.assignedDepartment?.name || 'None',
                contact_funnel: (fullContact as any)?.funnelStage?.name || 'None',
                ...
                instance_name: provider.instanceConfig.instanceName,
              ...
              if (edge) {
                  await executeStep(session!, flow, edge.target, text, provider, remoteJid, teamId, chatId);
              }
              return true;
          }
      }
    ...
    // 4. Ongoing Session Execution
    const flow = FlowDataSchema.parse({
        nodes: session.automation.nodes,
        edges: session.automation.edges
    });
    ...
    if (currentNode.type === 'collect') {
        ...
        if (selectedEdge) {
            ...
            await provider.sendText(remoteJid, { text: fallbackMsg });
            return true;
        }
    } 
    else {
    ...
    if (nextNodeId) {
        await executeStep(session, flow, nextNodeId, text, provider, remoteJid, teamId, chatId);
        return true;
    }

    return false;
  } catch (error: any) {
    console.error('[AutomationEngine_Fatal]', error.message);
    return false;
  }
}
...
async function executeStep(
    session: any, 
    flow: z.infer<typeof FlowDataSchema>, 
    nodeId: string,
    input: string, 
    ...
    if (nextNode.type === 'condition') {
        ...
        if (targetEdge) {
            ...
            await executeStep(updatedSession, flow, targetEdge.target, input, provider, remoteJid, teamId, chatId);
        } else {
             await db.update(automationSessions).set({ status: 'completed' }).where(eq(automationSessions.id, session.id));
             await pusherServer.trigger(`team-${teamId}`, 'chat-status-update', {
                 chatId, type: 'automation', status: 'completed'
    ...
    if (nextNode.type === 'delay') {
        ...
        await moveToNextAuto(updatedSession, flow, nextNode.id, provider, remoteJid, teamId, chatId);
    }
        ...
        await processTextOutput(nextNode, provider, remoteJid, teamId, chatId, variables);
        ...
        await processMediaOutput(nextNode, provider, remoteJid, teamId, chatId, variables);
        ...
        await provider.sendText(remoteJid, { text: text });
        _logNode({ collecting: nextNode.data.variable });
    }
    else if (nextNode.type === 'button_message' || nextNode.type === 'list_message') {
        await processInteractiveOutput(nextNode, provider, remoteJid, teamId, chatId, variables);
        ...
        await provider.sendText(remoteJid, { text: finalMsg });
        _logNode({ url });
        await moveToNextAuto(updatedSession, flow, nextNode.id, provider, remoteJid, teamId, chatId);
    }
        ...
        await processHttpRequest(nextNode, updatedSession, flow, provider, remoteJid, teamId, chatId);
    }
}
...
async function processHttpRequest(
    ...
    if (nextEdge) {
        ...
        await executeStep(session, flow, nextEdge.target, '', provider, remoteJid, teamId, chatId);
    } else {
        await db.update(automationSessions).set({ status: 'completed' }).where(eq(automationSessions.id, session.id));
...
async function moveToNextAuto(session: any, flow: FlowData, currentNodeId: string, provider: WhatsAppProvider, remoteJid: string, teamId: number, chatId: number) {
    const edge = flow.edges.find(e => e.source === currentNodeId);
    if (edge) {
        await new Promise(r => setTimeout(r, 500)); 
        await executeStep(session, flow, edge.target, '', provider, remoteJid, teamId, chatId);
    } else {
        await db.update(automationSessions).set({ status: 'completed' }).where(eq(automationSessions.id, session.id));
        await pusherServer.trigger(`team-${teamId}`, 'chat-status-update', {
            chatId, type: 'automation', status: 'completed'
...
async function processTextOutput(node: AutomationNode, provider: WhatsAppProvider, remoteJid: string, teamId: number, chatId: number, variables: Record<string, any>) {
    if (node.type === 'message') {
        ...
        text = replaceVariables(text, variables);
        await provider.sendText(remoteJid, { text });
    }
        ...
        await provider.sendText(remoteJid, { text: message });
    }
}
...
async function processMediaOutput(node: AutomationNode, provider: WhatsAppProvider, remoteJid: string, teamId: number, chatId: number, variables: Record<string, any> = {}) {
    ...
    try {
        const base64 = await fileToBase64(data.mediaUrl);
        if (!base64) return;
        ...
        const payload: SendMediaPayload = {
            ...
            mimetype: mimetype,
            fileName: path.basename(data.mediaUrl)
        };
        await provider.sendMedia(remoteJid, payload);
    } catch (e: any) {
        console.error("[Automation Engine] Media processing error:", e.message);
    }
}
...
async function processInteractiveOutput(node: AutomationNode, provider: WhatsAppProvider, remoteJid: string, teamId: number, chatId: number, variables: Record<string, any>) {
    ...
    const payload = {
        ...
        body: bodyText,
        footer: footerText,
        type: node.type === 'button_message' ? 'button' : 'list',
        buttons: (data.buttons as any[])?.map((b, idx) => ({ text: b.text, value: b.value || `btn-${idx}` })),
        items: (data.items as any[])?.map((i, idx) => ({ title: i.title, description: i.description, rowId: i.rowId || `item-${idx}` })),
...
async function processSaveContact(node: AutomationNode, session: any, teamId: number, chatId: number) {
    ...
    if (currentContact) {
        ...
        logCRMAction({ teamId, chatId, contactId: currentContact.id, action: 'automation_update', metadata: valuesToSet });
    } else {
        if (!valuesToSet.name) {
        ...
        if (newContact) {
            logCRMAction({ teamId, chatId, contactId: newContact.id, action: 'automation_create', metadata: valuesToSet });
        }
    }
}