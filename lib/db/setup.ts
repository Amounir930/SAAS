...
async function main() {
  // 0. Production Safeguard
  if (process.env.NODE_ENV === 'production' && process.env.ALLOW_RISKY_SETUP !== 'true') {
    console.error('CRITICAL: Setup script blocked in production environment.');
    console.error('#########################################################');
    console.error('ERROR: This script is restricted in production.');
    console.error('To override, set ALLOW_RISKY_SETUP=true');
    console.error('#########################################################');
    process.exit(1);
  }

  // 1. Existing .env Check
  try {
    await fs.access(path.join(process.cwd(), '.env'));
    const confirm = await question('A .env file already exists. Overwrite? (y/N): ');
