...
export const signIn = validatedAction(signInSchema, async (data, formData) => {
  ...
  const rl = rateLimit(`auth:sign-in:${ip}`, RATE_LIMITS.auth);
  if (!rl.success) {
    return { error: 'Too many attempts. Please try again later.', email, password };
  }
...
export const signUp = validatedAction(signUpSchema, async (data, formData) => {
  ...
  const hdrs = await headers();
  const ip = hdrs.get('x-forwarded-for')?.split(',')[0].trim() || hdrs.get('x-real-ip') || '127.0.0.1';
  const rl = rateLimit(`auth:sign-up:${ip}`, RATE_LIMITS.auth);
  if (!rl.success) {
    return { error: 'Too many attempts. Please try again later.', email, password };
  }
...
export const updateAccount = validatedActionWithUser(
  updateAccountSchema,
  async (data, _, user) => {
    const { name, email, enableSignature } = data;
    const userWithTeam = await getUserWithTeam(user.id);
    ...
    await Promise.all([
      db.update(users).set({
        name,
        email,
        enableSignature: enableSignature === 'on',
      }).where(eq(users.id, user.id)),
