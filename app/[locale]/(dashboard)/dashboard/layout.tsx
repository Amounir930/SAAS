...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const contextAgents: Agent[] = teamMembers?.teamMembers?.map(tm => ({
    ...tm.user,
  ...
  const validChats = useMemo(() => {
    ...
    return chats.filter((chat) => {
      ...
      if (detailedFilters.agentId) {
        if (Number(chat.contact?.assignedUser?.id) !== Number(detailedFilters.agentId)) return false;
      }
  ...
  }, [validChats, searchQuery, detailedFilters]);
  ...
  if (!mounted) {
    return <div className="flex h-screen bg-muted/40 dark:bg-background" />;
  }
