...
export function logCRMAction(params: BaseLogParams & {
  action?: string;
  eventType?: 'agent_assigned' | 'funnel_changed' | 'tag_added' | 'department_assigned' | string;
  ...
  enqueue({
    ...
    eventType: (params.eventType || params.action || 'crm_action') as any,
    automationId: params.automationId || null,
    automationSessionId: params.sessionId || null,
    nodeId: null,
    nodeType: null,
    inputData: null,
    outputData: null,
    metadata: safeTruncate(finalMetadata),
    durationMs: null,
    status: 'success',
    errorMessage: null,
  });
