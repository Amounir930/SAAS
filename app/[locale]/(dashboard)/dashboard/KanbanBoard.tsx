...
function getDaysInStage(dateString: string | null | undefined) {
    if (!dateString) return 0;
    const updated = new Date(dateString);
    if (isNaN(updated.getTime())) return 0;
    const now = new Date();
    const diffTime = Math.max(0, now.getTime() - updated.getTime());
...
export default function KanbanBoard() {
  ...
  const { data: chats, isLoading: loadingChats, mutate: mutateChats } = useSWR<ApiChat[]>('/api/chats?scope=kanban', fetcher);
    ...
    if (stages.length > 0 && chats) {
      ...
      chats.forEach(chat => {
        const card: ChatCard = {
            ...
            funnelStageId: chat.contact?.funnelStage?.id || null,
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    if (type === 'COLUMN') {
        ...
        try {
            ...
            // Small delay before global refresh to allow animation to finish
            setTimeout(() => globalMutate('/api/funnel-stages'), 100);
        } catch (e) {
  ...
  if (!mounted || loadingStages || loadingChats) return <div className="flex h-full items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-muted-foreground" /></div>;
...
function CardItem({ card, index, toggleAlert, t }: { card: ChatCard, index: number, toggleAlert: (c: ChatCard) => void, t: (key: string, values?: any) => string }) {
