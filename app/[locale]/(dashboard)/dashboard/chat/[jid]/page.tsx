...
export default function ChatPage() {
  ...
  const chatDetails: ChatDetails = {
    id: currentChat?.id.toString() || '0',
    jid: remoteJid || '',
    remoteJid: remoteJid || '',
    name: contact?.name || currentChat?.name || currentChat?.pushName || chatNumber || 'Chat',
    profilePicUrl: currentChat?.profilePicUrl || undefined,
    isGroup: isGroup,
    unreadCount: currentChat?.unreadCount || 0,
    lastTimestamp: currentChat?.lastMessageTimestamp ? new Date(currentChat.lastMessageTimestamp) : undefined
  };
  ...
  useEffect(() => {
    if (Array.isArray(messages) && messages.length > 0 && !searchQuery) {
  ...
  useEffect(() => {
    ...
    const channel = getTeamChannel(Number(teamId));
    if (!channel) return;
      ...
      if (payload.remoteJid === remoteJid && instanceMatch) {
        mutateMessages((currentMessages = []) => {
          ...
          const messageWithStatus: Message = { 
            ...
            jid: payload.remoteJid || remoteJid,
            fromMe: payload.fromMe || false,
            timestamp: payload.timestamp ? new Date(payload.timestamp) : new Date(),
            status: payload.status || (payload.fromMe ? 'sent' : 'delivered')
          } as Message;
          return [...(currentMessages || []), messageWithStatus];
        }, false);
    ...
    const handleMessageReaction = (payload: { messageId: string; chatId: number; emoji: string | null; fromMe: boolean; remoteJid: string | null; participantName: string | null; action: 'add' | 'remove' }) => {
      mutateMessages((currentMessages = []) => currentMessages.map(msg => {
        ...
        if (payload.action === 'add' && payload.emoji) {
          ...
          const newReaction: Reaction = { emoji: payload.emoji, count: 1, fromMe: payload.fromMe, remoteJid: payload.remoteJid, participantName: payload.participantName };
  ...
  useEffect(() => {
    if (Array.isArray(messages) && remoteJid && teamId) {
  ...
  const handleSendText = async (e: React.FormEvent) => {
    ...
    try {
      ...
      mutateMessages((currentMessages = []) => currentMessages.map(msg => msg.id === tempId ? { ...sentMessageData, timestamp: new Date(sentMessageData.timestamp) } : msg), false);
  ...
  const handleSendAttachment = async (file: File) => {
    ...
    const optimisticMessage: Message = {
        ...
        body: messageType === 'documentMessage' ? fileName : null as any, 
  ...
  const handleReact = async (messageId: string, emoji: string) => {
    ...
    mutateMessages((currentMessages = []) => currentMessages.map(msg => {
      ...
      if (emoji) {
        ...
        const newReaction: Reaction = { emoji, count: 1, fromMe: true, remoteJid: null, participantName: null };
        return { ...msg, reactions: [...filtered, newReaction] };
      } else {
  ...
  const showSyncBanner = !syncDismissed && currentChat?.instanceId && Array.isArray(messages) && messages.length === 0 && !isLoading && !error;
    ...
    <div className="flex h-screen bg-background">
      <div className="flex flex-col flex-1 h-screen">
        ...
        <ChatHeader
          ...
          chatAvatar={chatDetails.profilePicUrl}
          isGroup={isGroup}
          onBack={() => {}}
        ...
        <footer className="flex flex-col border-t bg-background shrink-0">
          <ChatInput
            ...
            onSendMessage={handleSendText as any}
            onVoiceRecorded={handleSendAudio as any}
      ...
      <TemplateDialog 
        ...
        onSend={handleSendTemplate as any}
        templates={[]} 
      />
