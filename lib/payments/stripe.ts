...
export async function createCheckoutSession(input: CreateCheckoutSessionInput) {
  const { team, priceId } = CreateCheckoutSessionSchema.parse(input);
...
export async function createCustomerPortalSession(team: typeof teams.$inferSelect) {
  ...
  if (configurations.data.length > 0) {
    ...
    if (!product.active) {
      logger.error('[Stripe] Product is not active for team:', { teamId: team.id, stripeProductId: team.stripeProductId });
    ...
    if (prices.data.length === 0) {
      logger.error('[Stripe] No active prices found for product:', { productId: product.id });
...
export async function handleSubscriptionChange(
  ...
  if (!team) {
    logger.error('[Stripe_Webhook] Team not found for customer:', { customerId });
    return;
  }
